...
import { User, LogOut, Settings as IconSettings, Heart, Menu, X, ShoppingCart } from 'lucide-react'
...
export default function Navbar() {
  ...
  const wishlistCount = typeof wishlistCtx.getTotalItems === 'function'
    ? wishlistCtx.getTotalItems()
    : (wishlistCtx.items?.length || 0)

  const cartCount = typeof cartCtx.getTotalItems === 'function'
    ? cartCtx.getTotalItems()
    : (cartCtx.items?.length || 0)

  // Cart items and total price calculation (fallback if context doesn't provide a helper)
  const cartItems = cartCtx.items || []
  const cartTotal = typeof cartCtx.getTotalPrice === 'function'
  ...
  */
  ...
  return (
    <nav className="sticky top-0 z-40 border-b border-black/5 dark:border-white/10" aria-label="Main navigation">
      ...
      <div className="backdrop-blur glass">
        <div className=" mx-auto px-2 h-16 flex items-center gap-4">
          {/* Logo / Home link */}
          <Link to="/" className="flex items-center gap-2 font-extrabold text-xl" onClick={onNavigate}>
          ...
          <div className="hidden md:flex items-center gap-2">
            <NavLink to="/" className={({isActive}) => `px-3 py-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/10 ${isActive ? 'font-semibold' : ''}`} onClick={onNavigate}>
              {t('navbar.home', 'Home')}
            </NavLink>

            <NavLink to="/courses" className={({isActive}) => `px-3 py-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/10 ${isActive ? 'font-semibold bg-black/5 dark:bg-white/10' : ''}`} onClick={onNavigate}>
              {t('navbar.courses', 'Courses')}
            </NavLink>

            <NavLink to="/dashboard" className={({isActive}) => `px-3 py-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/10 ${isActive ? 'font-semibold bg-black/5 dark:bg-white/10' : ''}`} onClick={onNavigate}>
          ...
          <div className="ms-auto flex items-center gap-2">
            ...
            <div ref={cartContainerRef} className="relative">
              ...
              >
                {/* Use an accessible icon and an aria-live badge for updates */}
                <ShoppingCart className="w-5 h-5" />
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold" aria-live="polite">
                    {cartCount}
                  </span>
                )}
              </button>

              {/* CartPanel is a separate component; we pass isMobile and i18n for RTL & sheet behavior */}
              <CartPanel
                open={cartOpen}
            ...
            <div className="hidden sm:block">
              {isAuthenticated ? (
                <div className="relative">
                  ...
                  >
                    <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center">
                      <span className="text-white text-sm font-bold">{displayName[0] || 'U'}</span>
                    </div>
                    <span className="hidden md:block text-sm font-medium">{displayName}</span>
                  </button>

                  {showUserMenu && (
                    <div id="user-menu" className="absolute right-0 top-full mt-2 w-48 glass rounded-2xl border border-white/20 p-2 shadow-lg" role="menu">
                      <Link to="/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-white/10 transition-colors" onClick={() => { setShowUserMenu(false); onNavigate(); }} role="menuitem">
                        <User className="w-4 h-4" />
                        {t('navbar.dashboard', 'Dashboard')}
                      </Link>

                      <button className="w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-white/10 transition-colors text-left" onClick={() => { /* settings */ }} role="menuitem">
                      ...
                      <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-red-500/20 hover:text-red-400 transition-colors text-left" role="menuitem">
                ...
                <Link to="/auth" className="px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all duration-200 font-medium" onClick={onNavigate}>
      ...
      <div className={`sm:hidden overflow-hidden bg-white dark:bg-gray-800 transition-[max-height] duration-300 ${mobileOpen ? 'max-h-[100vh]' : 'max-h-0'}`} aria-hidden={!mobileOpen}>
        <div className="px-4 pb-4 space-y-3">
          <NavLink to="/" onClick={onNavigate} className={({isActive}) => `block px-3 py-2 rounded-lg ${isActive ? 'font-semibold bg-black/5' : 'hover:bg-black/5'}`}>{t('navbar.home','Home')}</NavLink>

          <NavLink to="/courses" onClick={onNavigate} className={({isActive}) => `block px-3 py-2 rounded-lg ${isActive ? 'font-semibold bg-black/5' : 'hover:bg-black/5'}`}>{t('navbar.courses','Courses')}</NavLink>

          <NavLink to="/dashboard" onClick={onNavigate} className={({isActive}) => `block px-3 py-2 rounded-lg ${isActive ? 'font-semibold bg-black/5' : 'hover:bg-black/5'}`}>{t('navbar.dashboard','Dashboard')}</NavLink>

          <div className="border-t pt-3 flex flex-col gap-2">
            <button onClick={() => { switchLang(); setMobileOpen(false) }} className="text-left px-3 py-2 rounded-lg hover:bg-black/5">
              {t('navbar.language','Language')}: {i18n.language === 'ar' ? 'AR' : 'EN'}
            </button>

            <button onClick={() => { toggleTheme(); setMobileOpen(false) }} className="text-left px-3 py-2 rounded-lg hover:bg-black/5">
              {t('navbar.theme','Theme')}: {theme === 'dark' ? t('navbar.dark','Dark') : t('navbar.light','Light')}
            </button>

            <Link to="/wishlist" onClick={onNavigate} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-black/5">
              <span>{t('navbar.wishlist','Wishlist')}</span>
              {wishlistCount > 0 && <span className="bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold">{wishlistCount}</span>}
            </Link>

            {/* Cart link on mobile opens sheet */}
            <button onClick={() => { setCartOpen(true); setMobileOpen(false) }} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-black/5">
              <span>{t('navbar.cart','Cart')}</span>
              <div className="flex items-center gap-2">
                <CartIcon />
                {cartCount > 0 && <span className="bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold">{cartCount}</span>}
              </div>
            </button>
          </div>
          ...
          <div className="border-t pt-3">
            {isAuthenticated ? (
              <div className="space-y-2">
                ...
                <Link to="/dashboard" onClick={onNavigate} className="block px-3 py-2 rounded-lg hover:bg-black/5 flex items-center gap-2">
                  <User className="w-4 h-4" /> {t('navbar.dashboard','Dashboard')}
                </Link>

                <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-black/5 flex items-center gap-2">
                  <IconSettings className="w-4 h-4" /> {t('navbar.settings','Settings')}
                </button>

                <button onClick={handleLogout} className="w-full text-left px-3 py-2 rounded-lg hover:bg-red-500/10 text-red-600 flex items-center gap-2">
                  <LogOut className="w-4 h-4" /> {t('navbar.logout','Logout')}
                </button>
              </div>
            ) : (
              <Link to="/auth" onClick={onNavigate} className="block text-center px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl">
                {t('navbar.signIn','Sign In')}
              </Link>
            )}
          </div>
